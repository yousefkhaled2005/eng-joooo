...
# Optional
FACEBOOK_URL = ""  # مثال: "https://facebook.com/yourprofile"
...
def group_voices(voices):
    ...
    for v in voices:
        if v.get("Locale") and v.get("ShortName"):
            by_locale[v["Locale"]].append(v)

    for loc in by_locale:
        by_locale[loc] = sorted(by_locale[loc], key=lambda x: (x.get("Gender") or "", x.get("FriendlyName") or ""))

    return dict(sorted(by_locale.items(), key=lambda kv: kv[0]))


def make_data_audio_link(mp3_path: str) -> str:
    """Create a data: URL for preview link (works best for moderate file sizes)."""
...
# =========================
with st.sidebar:
    st.markdown("## 👨‍💻 المطور: يوسف خالد")
    st.caption("مهندس كمبيوتر وجرافيك ديزاينر متخصص في الـ AI و Web Mobile.")
...
<a class="social-btn whatsapp" href="{WHATSAPP_URL}" target="_blank">💬 تواصل واتساب</a>
""",
        unsafe_allow_html=True,
    )
    ...
    st.markdown("---")
    st.caption("جميع الحقوق محفوظة © 2026 لصالح المهندس يوسف خالد")
...
st.markdown(
    """
<div class="hero">
  <h1>🎙️ منصة المهندس يوسف لتحويل النص إلى صوت (AI)</h1>
  <p>تحويل ذكي، أصوات متعددة، وتحكم كامل في السرعة</p>
</div>
""",
    unsafe_allow_html=True,
)
...
# Load voices
# =========================
with st.spinner("جاري تحميل الأصوات..."):
...
with right:
    ...
    locale = st.selectbox(
        "🌍 اللغة/الدولة (Locale):",
        options=locale_list,
        index=locale_list.index(default_locale) if default_locale in locale_list else 0
    )
...
with left:
    st.markdown('<div class="glass">', unsafe_allow_html=True)
    st.markdown("### 📝 أدخل النص المراد تحويله هنا:")
    text = st.text_area(
        label="",
        height=260,
        placeholder="اكتب ما تريد تحويله لصوت احترافي...",
    )
